...
export default function Player({ track, onNext, onPrev }) {
  ...
  return (
    <>
      ...
      <FormControlLabel
        control={<Switch onChange={handleBlindtest} />}
        label="Blindtest"
        sx={{ position: "fixed", top: 12, left: 10 }}
      />
      <FormControlLabel
        control={<Switch onChange={handleKaraoke} />}
        label="Karaoke"
        sx={{ position: "fixed", top: 45, left: 10 }}
      />
      <Box sx={{ width: "100%", overflow: "hidden" }}>
        <Widget>
          <Box sx={{ display: "flex", alignItems: "center" }}>
            <CoverImage>
              <img alt="???" src={blindtest ? null : track.cover} />
            </CoverImage>
            ...
            >
              {paused ? (
                ...
                <PauseRounded
                  sx={{ fontSize: "3rem" }}
                  htmlColor={mainIconColor}
                  onClick={() => currentTrack.current.pause()}
                />
              )}
            </IconButton>
